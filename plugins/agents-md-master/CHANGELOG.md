# Changelog

All notable changes to AGENTS.md Master are documented in this file.

This changelog follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)
and [Semantic Versioning](https://semver.org/spec/v2.0.0.html). Use ISO 8601
dates (`YYYY-MM-DD`) and keep entries concise, user-facing, and actionable.

## [Unreleased]

### Changed

- Slimmed the skill entrypoint into an explicit reference router to reduce
  prompt-context pressure while preserving the full mode guidance in references.

## [0.2.0](https://github.com/nerymurillohnd/codex-essentials/compare/plugin/agents-md-master/v0.1.0...plugin/agents-md-master/v0.2.0) (2026-09-14)

### Features

- add semantic AGENTS governance ([2ed9204](https://github.com/nerymurillohnd/codex-essentials/commit/2ed920435b1bf9842ef9f80a5a27934642ccaee3))

### Added

- Semantic-governance audits for within-file and cross-chain duplication,
  contradictions, deletion candidates, root-density signals, writing quality,
  and linked-guidance integrity.
- Reviewable semantic ledgers, contradiction decision prompts, and evaluation
  fixtures that preserve intentional reinforcement and explicit controls.

## [0.1.0] - 2026-09-13

### Added

- Initial marketplace plugin for evidence-based Codex `AGENTS.md` system
  creation, audit, maintenance, refactoring, and scenario-based evaluation.
